...
export class Estimate_search_repairItem extends React.Component {
    constructor(props) {
        super(props);
    ...
    saveItem = (values) => {
        ...
        if (!itemId) {
            ...
            const estimate_search_repairItem = [{ ...item, item_id: itemId }]

            if (type === ITEM_EDIT) {
                removeEstimate_search_repairItem({ id: itemId })
            }

            setEstimate_search_repairItems({ estimate_search_repairItem })
    ...
    removeItem = () => {
        const { removeEstimate_search_repairItem, itemId, navigation, language } = this.props
    ...
    render() {
        ...
        return (
            <DefaultLayout
                headerProps={{
                    leftIconPress: () => navigation.navigate(ROUTES.ESTIMATE_SEARCH_REPAIR),
                    title: isCreateItem ?
