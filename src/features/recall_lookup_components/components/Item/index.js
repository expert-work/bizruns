...
export class Recall_lookup_componentItem extends React.Component {
    constructor(props) {
        super(props);
    ...
    saveItem = (values) => {
        ...
        if (!itemId) {
            ...
            const recall_lookup_componentItem = [{ ...item, item_id: itemId }]

            if (type === ITEM_EDIT) {
                removeRecall_lookup_componentItem({ id: itemId })
            }

            setRecall_lookup_componentItems({ recall_lookup_componentItem })
    ...
    removeItem = () => {
        const { removeRecall_lookup_componentItem, itemId, navigation, language } = this.props
    ...
    render() {
        ...
        return (
            <DefaultLayout
                headerProps={{
                    leftIconPress: () => navigation.navigate(ROUTES.RECALL_LOOKUP_COMPONENT),
                    title: isCreateItem ?
